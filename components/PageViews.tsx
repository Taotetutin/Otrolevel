...
export const QuizListView: React.FC<QuizListViewProps> = ({ quizzes }) => {
  ...
  console.log("[QuizListView] Rendering. CompletedQuizIds:", userProfile.completedQuizIds);

  const firstEverIncompleteQuizIndex = quizzes.findIndex(q => !userProfile.completedQuizIds.includes(q.id));
  console.log("[QuizListView] First ever incomplete quiz index:", firstEverIncompleteQuizIndex);
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {quizzes.map((quiz, index) => {
          ...
          if (!isCompleted) {
            if (firstEverIncompleteQuizIndex === -1) { 
              isActive = false; 
            } else if (index === firstEverIncompleteQuizIndex) {
              isActive = true;
            } else if (index > firstEverIncompleteQuizIndex) {
              isLocked = true;
            }
          ...
          let levelBadgeBgColor = 'bg-blue-100 text-blue-700';
          ...
          if (isLocked) {
            ...
            levelBadgeBgColor = 'bg-slate-300 text-slate-600';
          } else if (isCompleted) {
            ...
            statusBgColor = "bg-slate-100 text-slate-700";
            cardClassName += "border-2 border-slate-400 bg-white";
          }
          ...
          return (
            <div 
              ...
              aria-label={`Quiz: ${quiz.title}. Nivel ${index + 1}. Estado: ${isLocked ? 'Bloqueado' : isCompleted ? 'Completado' : 'Activo'}`}
            >
...
export const QuizTakingView: React.FC<QuizTakingViewProps> = ({ quizzes }) => {
  ...
  const resetQuizState = useCallback(() => {
    console.log(`[QuizTakingView ${quizId}] resetQuizState llamado.`);
  ...
  useEffect(() => {
    console.log(`[QuizTakingView ${quizId}] Efecto de carga del quiz. quizId: ${quizId}`);
    const foundQuiz = quizzes.find(q => q.id === quizId);
    if (foundQuiz) {
      console.log(`[QuizTakingView ${quizId}] Quiz encontrado: ${foundQuiz.title}. Completed IDs:`, userProfile.completedQuizIds);
      ...
      const isQuizAccessible = userProfile.completedQuizIds.includes(foundQuiz.id) || 
                              (firstIncompleteQuizIndex === -1 ? true : currentQuizIndexInGlobalList <= firstIncompleteQuizIndex);

      console.log(`[QuizTakingView ${quizId}] Es accesible? ${isQuizAccessible}. firstIncompleteIdx: ${firstIncompleteQuizIndex}, currentIdx: ${currentQuizIndexInGlobalList}`);

      if (!isQuizAccessible) {
        console.warn(`[QuizTakingView ${quizId}] Quiz ${quizId} no accesible. Redirigiendo.`);
        alert("Este quiz está bloqueado. Completa los anteriores primero.");
        navigate('/quizzes');
        return;
      }
      setCurrentQuiz(foundQuiz);
      // NO LLAMAR A resetQuizState() aquí. El QuizTakingViewWrapper maneja el reseteo
      // del estado al montar un nuevo componente cuando cambia el quizId (key).
      // Si el quizId es el mismo (ej. al marcar como completado), no queremos resetear
      // el estado aquí porque showResultsModal debe permanecer.
    } else {
      console.warn(`[QuizTakingView ${quizId}] Quiz con ID ${quizId} no encontrado. Redirigiendo a /quizzes.`);
      navigate('/quizzes'); 
    }
    return () => {
        console.log(`[QuizTakingView ${quizId}] Limpiando timers al desmontar o cambiar quizId.`);
        if (timerIntervalRef.current) clearInterval(timerIntervalRef.current);
        if (penaltyTimeoutRef.current) clearTimeout(penaltyTimeoutRef.current);
    }
  }, [quizId, quizzes, navigate, userProfile.completedQuizIds]); // resetQuizState eliminado de dependencias

  useEffect(() => {
    if (isAnswered || isTimeUp || showResultsModal || !currentQuiz || showIncorrectAnswerPenalty) {
      if (timerIntervalRef.current) clearInterval(timerIntervalRef.current);
      return;
    }
  ...
  }, [currentQuestionIndex, currentQuiz, isAnswered, isTimeUp, showResultsModal, questionKey, showIncorrectAnswerPenalty]);
  ...
  const handleAnswer = (optionId: string) => {
    if (isAnswered || !currentQuiz || showIncorrectAnswerPenalty) return;
  ...
  const handleNextQuestion = () => {
    ...
    console.log(`[QuizTakingView ${quizId}] handleNextQuestion. Current question index: ${currentQuestionIndex}, Total questions: ${currentQuiz.questions.length}`);
    ...
    if (currentQuestionIndex < currentQuiz.questions.length - 1) {
      ...
      console.log(`[QuizTakingView ${quizId}] Última pregunta respondida. Marcando quiz como completado.`);
      markQuizCompleted(currentQuiz.id);
      
      if (score >= (currentQuiz.questions.length * (currentQuiz.questions[0]?.points || 10) * 0.7)) { 
         console.log(`[QuizTakingView ${quizId}] Puntaje suficiente para XP. Score: ${score}`);
         addXp(score);
      } else {
         console.log(`[QuizTakingView ${quizId}] Puntaje NO suficiente para XP. Score: ${score}`);
      }
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in relative">
      ...
      {!isTimeUpOverlayVisible && !showIncorrectAnswerPenalty && !showResultsModal && currentQuiz && renderQuizContent()}
      
      <Modal isOpen={showResultsModal} onClose={() => { setShowResultsModal(false); navigate('/quizzes'); }} title="¡Quiz Completado!">
        <div className="text-center">
          <Icon path={TROPHY_ICON_PATH} className="w-16 h-16 text-yellow-400 mx-auto mb-3 animate-bounce" viewBox="0 0 20 20"/>
          ...
          <div className="mt-6 space-y-3 sm:space-y-0 sm:flex sm:space-x-3 justify-center">
            <Button onClick={() => { setShowResultsModal(false); navigate('/quizzes'); }} variant="primary" className="w-full sm:w-auto">
              Lista de Quizzes
            </Button>
          </div>
        </div>
